...
class Nilai_m extends CI_Model
{
    ...
    public static $kategori = 'mapel_kategori';
    ...
    public function getDataKategoriNilai()
    {
        return $this->db
            ->where('id NOT IN(7)')
            ->get("mapel_kategori")->result();
    }
    public function getDataNilai($idtahun_akademik, $semester, $idkelas, $idsiswa)
    {
        return $this->db->select('x.*,x1.mapel_nama,x1.kategori')
            ->join('mapel x1', 'x1.idmapel = x.idmapel', 'left')
            ->where(['x.idtahun_akademik' => $idtahun_akademik, 'x.semester' => $semester, 'x.idkelas' => $idkelas, 'x.idsiswa' => $idsiswa])
            ->get(self::$table . ' x')->result_array();
    }
    public function getDataNilaiByKategori($idtahun_akademik, $semester, $idkelas, $idsiswa, $kategori)
    {
        return $this->db->select('x.*,x1.mapel_nama,x1.kategori')
            ->join('mapel x1', 'x1.idmapel = x.idmapel', 'left')
            ->where(['x.idtahun_akademik' => $idtahun_akademik, 'x.semester' => $semester, 'x.idkelas' => $idkelas, 'x.idsiswa' => $idsiswa, 'x1.kategori' => $kategori])
            ->get(self::$table . ' x')->result_array();
    }
    public function getDataNilaiEskul($idtahun_akademik, $semester, $idkelas, $idsiswa)
    {
        return $this->db->select('x.*,x1.mapel_nama,x1.kategori')
            ->join('mapel x1', 'x1.idmapel = x.idmapel', 'left')
            ->where(['x.idtahun_akademik' => $idtahun_akademik, 'x.semester' => $semester, 'x.idkelas' => $idkelas, 'x.idsiswa' => $idsiswa, 'x1.kategori' => 7])
            ->get(self::$table . ' x')->result_array();
    }
