<script>
    ...
    function getFromSourceList() {
        ...
        let tahun_asal = $('[name="tahun_asal"]').val();
        let kelas_asal = $('[name="kelas_asal"]').val();
        $.ajax({
            type: "POST",
            data: {
    ...
    function getFromTargetList() {
        ...
        let tahun_tujuan = $('[name="tahun_tujuan"]').val();
        let kelas_tujuan = $('[name="kelas_tujuan"]').val();
        $.ajax({
            type: "POST",
            data: {
    ...
    function saveToDestinationClass() {
        ...
        if (kelas_asal == kelas_tujuan && tahun_asal == tahun_tujuan) {
            ...
            let rows = $('input[name="asal"]:checked');
            let siswa_id = [];
            rows.each(function() {
    ...
    function DeleteFromDestinationClass() {
        let kelas_tujuan = $('[name="kelas_tujuan"]').val();
        let tahun_tujuan = $('[name="tahun_tujuan"]').val();
        let rows = $('input[name="tujuan"]:checked');
        let siswa_id = [];
        rows.each(function() {
            siswa_id.push($(this).val());
        });
...
<section class="content">
    <div class="row">
        <div class="col-md-6">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <button type="submit" class="btn btn-sm btn-primary btn-flat pull-right" onclick="saveToDestinationClass()"><i class="fa fa-arrow-right"></i>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" style="width: 100%;" name="kelas_asal" onchange="showAcademicYear()">
                                        <!-- <option value="unset">Belum Diatur</option> -->
                                        <option value="all">Tampilkan Semua</option>
                                        <?php foreach (list_kelas() as $row) : ?>
                                            <option value="<?= $row->idkelas; ?>">
                                                <?= $row->kelas_kd . ' - ' . $row->kelas_nama; ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                            ...
                            <div class="col-md-6" id="tahun_asal">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" style="width: 100%;" name="tahun_asal" onchange="getFromSourceList()">
                                        <?php foreach (list_academic_year() as $row) : ?>
                                            <option value="<?= $row->idtahun_akademik; ?>">
                                                <?= $row->tahun_akademik . " " . $row->semester; ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
        ...
        <div class="col-md-6">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <button class="btn btn-sm btn-danger btn-flat pull-right" onclick="DeleteFromDestinationClass()"><i class="fa fa-trash"></i> HAPUS</button>
                    <br>
                    <br>
                    <form action="" method="post">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label>Tahun Pelajaran</label>
                                    <select class="form-control select2" style="width: 100%;" name="tahun_tujuan" onchange="getFromTargetList()">
                                        <?php foreach (list_academic_year() as $row) : ?>
                                            <option value="<?= $row->idtahun_akademik; ?>">
                                                <?= $row->tahun_akademik . " " . $row->semester; ?>
                                            </option>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" style="width: 100%;" name="kelas_tujuan" onchange="getFromTargetList()">
                                        <?php foreach (list_kelas() as $row) : ?>
                                            <option value="<?= $row->idkelas; ?>">
                                                <?= $row->kelas_kd . ' - ' . $row->kelas_nama; ?>
                                            </option>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" style="width: 100%;" name="idmapel" required>
                                    <?php foreach (list_mapel() as $row) : ?>
                                        <option value="<?= $row->idmapel; ?>">
                                            <?= $row->mapel_kd . ' - ' . $row->mapel_nama; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control select2" style="width: 100%;" name="idtahun_akademik" required>
                                    <?php foreach (list_academic_year() as $row) : ?>
