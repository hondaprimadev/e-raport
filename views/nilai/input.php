<script>
    ...
    function pengetahuan(x) {
        let kkm = parseInt($('[name="kkm"]').val());
        let na = parseInt($('[name="pts' + x + '"]').val() || 0);
        ...
        console.log(selisih_kkm);
    }

    function ketrampilan(x) {
        let kkm = parseInt($('[name="kkm"]').val());
        let na = parseInt($('[name="uas' + x + '"]').val() || 0);

        let selisih_kkm = (100 - kkm) / 3;

        if (parseInt(na) < kkm) {
            $('[name="gradeKetrampilan' + x + '"]').val("D");
            console.log("D");
        } else if (parseInt(na) >= kkm && parseInt(na) <= Math.round(kkm + selisih_kkm) - 1) {
            $('[name="gradeKetrampilan' + x + '"]').val("C");
            console.log("C");
        } else if (parseInt(na) >= Math.round(kkm + selisih_kkm) && parseInt(na) <= Math.round(kkm + (selisih_kkm * 2)) -
            1) {
            $('[name="gradeKetrampilan' + x + '"]').val("B");
            console.log("B");
        } else if (parseInt(na) >= Math.round(kkm + (selisih_kkm * 2)) && parseInt(na) <= (kkm + (selisih_kkm *
                3))) {
            $('[name="gradeKetrampilan' + x + '"]').val("A");
            console.log("A");
        }
    }
                ...
                <form class="form-horizontal" action="<?= base_url('nilai/save_kkm/') . $rowI->idmengajar; ?>" method="post">
                    <div class="form-group">
                        ...
                        <div class="col-xs-4">
                            ...
                            <input type="number" class="form-control" id="kkm" name="kkm" value="<?= $rowI->kkm; ?>" max="100" placeholder="0" onkeyup="this.value = minmax(this.value,0,100)">
                        </div>
                ...
                <table class="table table-bordered table-striped table-hover" style="table-layout:fixed;">
                    <thead>
                        <tr>
                            ...
                            <th colspan="2" class="text-center" width="250">MODUL</th>
                            <th colspan="2" class="text-center" width="250">GRADE</th>
                        ...
                        foreach (siswa_by_kelas(_active_years()->idtahun_akademik, _active_years()->semester, $rowI->idkelas) as $row) : ?>
                            <?php foreach (nilai_by_siswa(_active_years()->idtahun_akademik, _active_years()->semester, $rowI->idkelas, $rowI->idmapel, $row->idsiswa) as $nilai) : ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" class="form-control" id="pts" name="pts<?= $row->idsiswa; ?>" min="0" max="100" placeholder="0" value="<?= $nilai->nilai_pts; ?>" onkeyup="this.value = minmax(this.value,0,100)" onchange="pengetahuan(<?= $row->idsiswa; ?>)">
                                    </td>
                                    <td>
                                        <input type="number" class="form-control" id="uas" name="uas<?= $row->idsiswa; ?>" min="0" max="100" placeholder="0" value="<?= $nilai->nilai_uas; ?>" onkeyup="this.value = minmax(this.value,0,100)" onchange="ketrampilan(<?= $row->idsiswa; ?>)">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control" id="grade" name="grade<?= $row->idsiswa; ?>" value="<?= $nilai->nilai_huruf; ?>" readonly>
                                    </td>
                                    <td>
                                        <input type="text" class="form-control" id="gradeKetrampilan" name="gradeKetrampilan<?= $row->idsiswa; ?>" value="<?= $nilai->nilai_huruf_ketrampilan; ?>" readonly>
                                    </td>
                                    <!-- <td>
                                        <input type="text" class="form-control" id="deskripsi" name="deskripsi<?= $row->idsiswa; ?>" value="<?= $nilai->deskripsi; ?>" readonly>
                                    </td> -->
                                </tr>
                            <?php endforeach; ?>
                        <?php endforeach; ?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <ol>
                    ...
                    <li>Untuk menutup informasi ini silahkan klik Tutup dan untuk membuka kembali anda bisa klik icon <i class="fa fa-edit"></i> atau Refresh browser anda</li>
                </ol>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default btn-flat btn-sm pull-right" data-dismiss="modal"><i class="fa fa-remove"></i> Tutup</button>
            </div>
        </div>
    </div>
</div>